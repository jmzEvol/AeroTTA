...
class StoreOriginalImage:
    """在 DataLoader worker 进程里把已加载的原始图（BGR numpy）
    存入 results['ori_img']，供 predict() 直接使用，
    避免在主进程推理时重复读硬盘导致死锁。
    """

    def transform(self, results: dict) -> dict:
        # results['img'] 是 LoadImageFromFile 加载的 BGR uint8 numpy array
...
@DATASETS.register_module()
class AeroScapesDataset(BaseSegDataset):
    ...
    METAINFO = dict(
        ...
        palette=[
            ...
            [0, 0, 230], [0, 60, 100], [255, 153, 0], [153, 153, 153],
            [70, 70, 70], [107, 142, 35], [128, 64, 128], [70, 130, 180]])
...
@DATASETS.register_module()
class UDD6Dataset(BaseSegDataset):
    """UDD6 dataset."""

    METAINFO = dict(
        classes=('other', 'facade', 'road', 'vegetation', 'vehicle', 'roof'),
        palette=[[0, 0, 0], [102, 102, 156], [128, 64, 128],
                 [107, 142, 35], [0, 0, 142], [70, 70, 70]])
