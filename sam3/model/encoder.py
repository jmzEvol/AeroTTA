...
from .lora import LoRAMultiheadAttention
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward_pre(
        ...
        tgt2 = self.norm2(tgt)
        # cross_attn_image 在注入 LoRA 后会自动处理权重级增量
        # (不注入时就是原始 MultiheadAttention，行为不变)
        tgt2 = self.cross_attn_image(
...
class TransformerEncoderFusion(TransformerEncoder):
    ...
    def inject_lora(self, rank: int = 4, alpha: float = 1.0,
                     ...
                     adapt_key: bool = False) -> None:
        """
        向指定层的 cross-attention 注入 LoRA 适配器。

        参数:
            rank:          LoRA 的秩 (r)，默认 4
            alpha:         缩放因子，默认 1.0
            layer_indices: 要注入 LoRA 的层索引列表，
                           默认为后半层 (例如 6 层 encoder 则注入第 3,4,5 层)
        """
        if layer_indices is None:
            # 默认注入后半部分层
            layer_indices = list(range(self.num_layers // 2, self.num_layers))

        if rank <= 0:
            raise ValueError("LoRA rank must be positive")
        layer_ranks = dict(layer_ranks or {})
        unknown_layers = sorted(set(layer_ranks) - set(layer_indices))
        if unknown_layers:
            unknown_text = ", ".join(str(layer) for layer in unknown_layers)
            raise ValueError(f"LoRA rank overrides reference uninjected layers: {unknown_text}")
        if any(layer_rank <= 0 for layer_rank in layer_ranks.values()):
            raise ValueError("LoRA layer ranks must be positive")
        base_scaling = float(alpha) / int(rank)

        for idx in layer_indices:
            layer = self.layers[idx]
            # 用 LoRAMultiheadAttention 替换 cross_attn_image:
            # 内部持有原始 MHA（已冻结），新增的 A/B 矩阵默认 requires_grad=True
            device = next(layer.parameters()).device
            layer_rank = int(layer_ranks.get(idx, rank))
            layer_alpha = base_scaling * layer_rank
            layer.cross_attn_image = LoRAMultiheadAttention(
                layer.cross_attn_image,
                rank=layer_rank,
                alpha=layer_alpha,
            ).to(device)

        # 先创建完所有层的 Q/V，再统一追加 K。这使开启 K 前后
        # 各层 Q/V 的随机初始值完全一致，便于与历史 QV 结果公平对比。
        if adapt_key:
            for idx in layer_indices:
                self.layers[idx].cross_attn_image.enable_key_lora()

    def get_lora_parameters(self) -> List[nn.Parameter]:
        """
        收集所有 LoRA 参数，用于构建优化器（只训练这些参数）。
        ...
        params = []
        for layer in self.layers:
            if isinstance(layer.cross_attn_image, LoRAMultiheadAttention):
                params.extend(layer.cross_attn_image.get_lora_parameters())
        return params

    def get_lora_parameters_by_layer(self) -> Dict[int, List[nn.Parameter]]:
        """Collect trainable LoRA parameters keyed by fusion-encoder layer."""
        params_by_layer = {}
        for layer_index, layer in enumerate(self.layers):
            if isinstance(layer.cross_attn_image, LoRAMultiheadAttention):
                params_by_layer[layer_index] = layer.cross_attn_image.get_lora_parameters()
        return params_by_layer

    def reset_lora_parameters(self) -> None:
        """
        重置所有 LoRA 参数到初始状态（A 用 Kaiming，B 用零），
        用于在 TTA 中处理新图像前重置适配器。
        """
        for layer in self.layers:
            if isinstance(layer.cross_attn_image, LoRAMultiheadAttention):
                layer.cross_attn_image.reset_lora()
    ...
    def save_lora(self, path: str) -> None:
        ...
        参数:
            path: 保存路径，如 "work_dirs/lora_epoch001.pt"
        """
        state = {}
        for i, layer in enumerate(self.layers):
            if isinstance(layer.cross_attn_image, LoRAMultiheadAttention):
                state[f"layer_{i}"] = {
                    "lora_q_A": layer.cross_attn_image.lora_q_A.state_dict(),
                    "lora_q_B": layer.cross_attn_image.lora_q_B.state_dict(),
                    "lora_v_A": layer.cross_attn_image.lora_v_A.state_dict(),
                    "lora_v_B": layer.cross_attn_image.lora_v_B.state_dict(),
                }
                if layer.cross_attn_image.adapt_key:
                    state[f"layer_{i}"]["lora_k_A"] = (
                        layer.cross_attn_image.lora_k_A.state_dict()
                    )
                    state[f"layer_{i}"]["lora_k_B"] = (
                        layer.cross_attn_image.lora_k_B.state_dict()
                    )
        torch.save(state, path)

    def load_lora(self, path: str) -> None:
        """
        从文件加载 LoRA 参数（需先调用 inject_lora 注入结构）。

        参数:
            path: 权重文件路径（由 save_lora 保存）
        """
        ...
        for i, layer in enumerate(self.layers):
            ...
            if key not in state:
                continue
            assert isinstance(layer.cross_attn_image, LoRAMultiheadAttention), (
                f"第 {i} 层未注入 LoRA，请先调用 inject_lora()"
            )
            layer.cross_attn_image.lora_q_A.load_state_dict(state[key]["lora_q_A"])
            layer.cross_attn_image.lora_q_B.load_state_dict(state[key]["lora_q_B"])
            layer.cross_attn_image.lora_v_A.load_state_dict(state[key]["lora_v_A"])
            layer.cross_attn_image.lora_v_B.load_state_dict(state[key]["lora_v_B"])
            if (
                layer.cross_attn_image.adapt_key
                and "lora_k_A" in state[key]
                and "lora_k_B" in state[key]
            ):
                layer.cross_attn_image.lora_k_A.load_state_dict(state[key]["lora_k_A"])
                layer.cross_attn_image.lora_k_B.load_state_dict(state[key]["lora_k_B"])
            layer.cross_attn_image._cached_W_eff = None  # 清除旧缓存
        print(f"[LoRA] 已从 {path} 加载权重")
